...
The described overheads in {{handshake}} and {{record}} are independent of the underlying transport as they do not consider DTLS handshake message fragmentation, how to compose DTLS handshake messages into records, and how the underlying layers influence the size of application plaintext sizes. The complete overhead for all layers depends on the combination of layers as well as assumptions regarding the devices and applications and are out of scope of the document. This section give a short overview of the overheads of UDP, TCP, and CoAP.
